...
public class ArisaKeyboard extends MeleeWeapon {
	...
	public int proc(Char attacker, Char defender, int damage ) {
		...
		for (Mob mob2 : Dungeon.level.mobs.toArray(new Mob[0])) {
			if (mob2.pos == attacker.pos + Dungeon.level.width()){
		...
		for (Mob mob3 : Dungeon.level.mobs.toArray(new Mob[0])) {
			if (mob3.pos == attacker.pos - Dungeon.level.width()*2){
		...
		for (Mob mob4 : Dungeon.level.mobs.toArray(new Mob[0])) {
			if (mob4.pos == attacker.pos + Dungeon.level.width()*2){
		...
		for (Mob mob5 : Dungeon.level.mobs.toArray(new Mob[0])) {
			if (mob5.pos == attacker.pos - Dungeon.level.width()*3){
		...
		for (Mob mob6 : Dungeon.level.mobs.toArray(new Mob[0])) {
			if (mob6.pos == attacker.pos + Dungeon.level.width()*3){
		...
		for (Mob mob7 : Dungeon.level.mobs.toArray(new Mob[0])) {
			if (mob7.pos == attacker.pos - Dungeon.level.width()*4){
		...
		for (Mob mob8 : Dungeon.level.mobs.toArray(new Mob[0])) {
			if (mob8.pos == attacker.pos + Dungeon.level.width()*4){
